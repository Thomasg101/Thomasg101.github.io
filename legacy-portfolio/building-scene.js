...
class BuildingScene extends HTMLElement {
  ...
  _build() {
    ...
    scene.add(hoarding);
    scene.add(new THREE.Mesh(mergeGeos(postParts), m.propsMetal));
    ...
    for (let i = 0; i < 13; i++) {
      ...
      this.workers.push({
        g, l1, l2, a1, a2,
        ox: (rnd() - 0.5) * (W - 3), oz: (rnd() - 0.5) * (D - 3),
        tx: (rnd() - 0.5) * (W - 3), tz: (rnd() - 0.5) * (D - 3),
        lag: i % 3, ph: rnd() * 6.28, ground: i >= 9,
        gr: 15 + rnd() * 13, ga: rnd() * 6.28
      });
    }
  ...
  _tick(now) {
    ...
    const cine = this.reduced ? 0 : easeIO(c01(1 - (now - this.t0) / 5400)) * c01(1 - p / 0.05);
    const fin = easeIO(c01((p - 0.84) / 0.16));
    ...
    const loadY = lerp(1.5, Math.max(3, topYc + 2.5), cycle);
    ...
    for (let i = 0; i < this.workers.length; i++) {
      ...
      if (wk.ground) {
        ...
        wk.g.position.set(Math.cos(a) * wk.gr, 0.2, Math.sin(a) * wk.gr * 0.72);
        wk.g.rotation.y = Math.atan2(-Math.sin(a) * spin, Math.cos(a) * 0.72 * spin);
        continue;
      }
    ...
    // ---- camera ----
    const e = easeIO(p);
    let theta = -0.7 + p * Math.PI * 2.45 + t * 0.012;
    let radius = lerp(80, 48, e) + Math.sin(p * Math.PI * 2) * 6;
    let camY = lerp(11, topYc + 20, Math.pow(p, 0.82));
    let aimY = lerp(3, topYc * 0.58 + 4, e);
    let fov = 38;
    ...
    this.pointerSm.x += (this.pointer.x - this.pointerSm.x) * Math.min(1, dt * 2.4);
    this.pointerSm.y += (this.pointer.y - this.pointerSm.y) * Math.min(1, dt * 2.4);
    theta += this.pointerSm.x * 0.05;
    camY += this.pointerSm.y * 2.6;
    aimY -= this.pointerSm.y * 1.4;

    this.camPos.set(Math.cos(theta) * radius, camY, Math.sin(theta) * radius);
    this.camAim.set(0, aimY, 0);
    const k = this.reduced || this._first ? 1 : 1 - Math.exp(-dt * 3.1);
    this._first = false;
    this.cam.position.lerp(this.camPos, k);
    this.cam.lookAt(this.camAim);
